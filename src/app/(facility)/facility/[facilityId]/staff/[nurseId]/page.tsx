...
import NurseProfileClient, { type NurseProfileData } from '@/app/(agency)/agency/[agencyId]/staff/[nurseId]/NurseProfileClient'

export default async function FacilityNurseProfilePage({
  params,
}: {
  params: Promise<{ facilityId: string; nurseId: string }>
}) {
  const { facilityId, nurseId } = await params
  ...
  if (!houseAgency) notFound()

  const agencyId = houseAgency.id
  ...
  return <NurseProfileClient data={data} backHref={`/facility/${facilityId}/staff`} />
}

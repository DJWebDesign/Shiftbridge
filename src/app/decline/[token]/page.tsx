...
import DeclineTokenClient from './DeclineTokenClient'
...
async function getTokenData(token: string): Promise<TokenPageData | null> {
...
export default async function DeclineTokenPage({ params }: { params: Promise<{ token: string }> }) {
  const { token } = await params
  const info = await getTokenData(token)
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="bg-white rounded-2xl border border-gray-200 p-8 max-w-md w-full text-center">
          ...
          <p className="text-gray-500 text-sm">This link is not valid or has already been used.</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="bg-white rounded-2xl border border-gray-200 p-8 max-w-md w-full text-center">
          ...
          <p className="text-gray-500 text-sm">This link expired 7 days after it was sent.</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="bg-white rounded-2xl border border-gray-200 p-8 max-w-md w-full text-center">
          ...
          <p className="text-gray-500 text-sm">This shift has been canceled.</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="bg-white rounded-2xl border border-gray-200 p-8 max-w-md w-full text-center">
          ...
          <p className="text-gray-500 text-sm">This link has already been used.</p>
        </div>
      </div>
    )
  }

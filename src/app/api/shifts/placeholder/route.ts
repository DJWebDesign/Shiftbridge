...
export async function POST(request: NextRequest) {
  ...
  if (!placeholder_facility_id || !credential_required || !shift_date || !start_time || !end_time) {
    return NextResponse.json(
      { error: 'placeholder_facility_id, credential_required, shift_date, start_time, end_time are required' },
      { status: 400 }
    )
  }
  ...
  const tier = Number(priority_tier ?? 1)
  ...
  // Verify the placeholder facility belongs to this agency
  const { data: placeholder } = await admin
    .from('placeholder_facilities')
    .select('id, agency_id')
    .eq('id', placeholder_facility_id)
    .single()
  ...
  if (error) {
    console.error('[shifts/placeholder POST] error:', error)
